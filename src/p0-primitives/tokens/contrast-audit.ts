// @ts-nocheck — Node-only script (runs via tsx); uses fs/path/url/process
...
export function parseRgb(func: string): RGB | null {
...
/** Parse HSLA colors */
export function parseHsl(func: string): RGB | null {
  const match = func.match(
 ...
 */
export function resolveVarValue(value: string, variables: Map<string, string>, depth = 0): string {
...
async function main(): Promise<number> {
  ...
  const cssFiles = [
    ...
    path.resolve(__dirname, './validity-tokens.css'),
  ];
...
if (process.argv[1] && process.argv[1].includes('contrast-audit')) {
