...
export class CanvasRenderContext implements RenderContext {
  ...
  pushTransform(matrix: DOMMatrix | number[]): void {
    const mat = Array.isArray(matrix) ? matrix : null;
    const domMat = Array.isArray(matrix) ? null : matrix;
    this.commands.push({ type: 'transform', action: 'push', matrix: mat, domMatrix: domMat });
  }

  popTransform(): void {
    this.commands.push({ type: 'transform', action: 'pop' });
  }
    ...
    for (const cmd of sorted) {
      if (cmd.type === 'rect') {
        ...
        if (cmd.action === 'push') {
          c.save();
          if (cmd.domMatrix) {
            c.setTransform(cmd.domMatrix as DOMMatrix);
          } else if (cmd.matrix) {
