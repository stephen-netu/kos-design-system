<script lang="ts">
  import type { AssayClearanceView, AssaySignalView } from '../../epistemic/types.js';
  interface Props {
  ...
  function signalLabel(signal: AssaySignalView): string {
    return signal.id;
  }
...
<div class="ds-assay-detail {className}">
  ...
  <div class="header">
    ...
    <div class="header-row">
      ...
      <span class="placeholder-badge">{clearance.gate}</span>
  ...
  {#if hardGates.length > 0}
    <div class="section">
      ...
      {#each hardGates as signal}
         <span class="placeholder-signal">{signalLabel(signal)}</span>
       {/each}
    </div>
  {/if}
