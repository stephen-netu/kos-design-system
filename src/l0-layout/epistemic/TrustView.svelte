    ...
    {#each getGroup(activeGroup) as item}
      ...
      >
        <div class="item-gauges">
          <span class="placeholder-gauge">S{item.allay.stage}</span>
           <span class="placeholder-badge">{item.assay?.gate ?? '—'}</span>
        </div>
        <div class="item-info">
          <span class="item-title">{item.title}</span>
          <span class="item-source">{item.source}</span>
        </div>
        {#if item.expert}<span class="placeholder-badge-sm"></span>{/if}
      </button>
    {/each}
