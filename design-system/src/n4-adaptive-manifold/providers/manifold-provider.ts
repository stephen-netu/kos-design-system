...
export class ManifoldProvider implements AlgorithmProvider {
  ...
  async layout(
    ...
    // Agents occupy the inner disk (35% radius) so they never land on top of
    // task clusters, which live on the full outer disk (100% radius).
    // Deterministic minimum-separation prevents overlap when multiple agents
    // project to nearby disk positions.
    const AGENT_DISK_SCALE = 0.35;
    const AGENT_MIN_SEPARATION = 72; // px — minimum center-to-center distance
    const agentRawPositions: Array<{ id: string; x: number; y: number; w: number; h: number; proj: { x: number; y: number } }> = [];
    for (const agent of agents) {
      ...
      agentRawPositions.push({ id: agent.id, x, y, w: size.w, h: size.h, proj });
    }

    // Deterministic repulsion: nudge apart agents closer than AGENT_MIN_SEPARATION.
    // Uses a fixed number of iterations (S-05 bounded, S-02 deterministic).
    for (let iter = 0; iter < 8; iter++) {
      let anyMoved = false;
      for (let i = 0; i < agentRawPositions.length; i++) {
        for (let j = i + 1; j < agentRawPositions.length; j++) {
          const a = agentRawPositions[i];
          const b = agentRawPositions[j];
          const acx = a.x + a.w / 2;
          const acy = a.y + a.h / 2;
          const bcx = b.x + b.w / 2;
          const bcy = b.y + b.h / 2;
          ...
          if (dist < AGENT_MIN_SEPARATION && dist > 0.01) {
            const overlap = (AGENT_MIN_SEPARATION - dist) / 2;
            ...
            const angle = (i * Math.PI * 2) / agentRawPositions.length;
            const nx = Math.cos(angle) * AGENT_MIN_SEPARATION / 2;
            const ny = Math.sin(angle) * AGENT_MIN_SEPARATION / 2;
            a.x -= nx;
            a.y -= ny;
            b.x += nx;
            b.y += ny;
    ...
    for (const ap of agentRawPositions) {
      positions.set(ap.id, {
        id: ap.id,
        x: ap.x,
        y: ap.y,
