...
>
  {#if name === 'power-node'}
    ...
    <circle cx="20" cy="20" r="8" fill="currentColor"/>
     <circle cx="17" cy="17" r="3" fill="var(--color-text-inverse)" opacity="0.3"/>
  ...
  {:else if name === 'connection'}
    ...
    <!-- Data packet -->
     <circle cx="12" cy="20" r="2" fill="var(--color-text-inverse)">
  ...
  {:else if name === 'power-plant'}
    ...
    {#if animation === 'steam'}
      <!-- Steam plumes -->
      <g stroke="var(--color-accent-hover)" stroke-width="2" stroke-linecap="round" fill="none" opacity="0.6">
